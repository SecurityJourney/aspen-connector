...
Neither Mode C nor D needs any git or API write access — Mode C calls the Adapt API and Mode D calls Guardian's extract-cwes endpoint, both just to record CWEs.
...
| `ASPEN_ENFORCE_GATE`                | No         | `false`                   | Enables the external access control gate check. Can run standalone (Mode E) or as a pre-check before Modes A-D.                                                                                        |
...
When `ASPEN_GATE_COMMENT_ON_FAILURE` is `true` (the default) and the gate blocks on a pull/merge request, the connector posts the failure reason returned by the gate endpoint as a comment. Comment failures are logged as warnings and never override the gate result.
