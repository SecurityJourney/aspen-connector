...
describe('GitHubProvider.getMetadata()', () => {
  ...
  it('reads committerEmail from git log', async () => {
    gitLogEmail = 'author@example.com';
    setGitHubEnv();
    const meta = await new GitHubProvider().getMetadata();
    assert.equal(meta.committerEmail, 'author@example.com');
  });

  it('throws when git log cannot determine the committer email', async () => {
    gitLogEmail = null;
    setGitHubEnv();
    await assert.rejects(
      () => new GitHubProvider().getMetadata(),
